...
class VtigerConnector {
    ...
    constructor(settings) {
        this.Log = require(process.argv[1] + settings.loggerPath);
    ...
    request(session, currentModel, operation, data, next) {
        ...
        this.Http.request(options, (error, result, body) => {
            ...
            try {
                ...
                this.Log.error('[' + currentModel + '][vtiger-connector.request] request error : ' + error);
                this.Log.error('[' + currentModel + '][vtiger-connector.request] request error message: ' + e.message);
                next(e, null);
            }
        });
    }
